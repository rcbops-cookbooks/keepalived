...
_add_nat_rules() {
  ...
  iptables-save -t nat | grep -s ".*PREROUTING -d $vip/32 -j DNAT --to-destination $src" > /dev/null
  RET=$?
  if [[ $RET != 0 ]]; then
  ...
  iptables-save -t nat | grep -s ".*OUTPUT -d $vip/32 -o lo -j DNAT --to-destination $src" > /dev/null
  RET=$?
  if [[ $RET != 0 ]]; then
